...
class SyntheticDataset(BasicDataset):
    def __init__(self, images_dir, masks_dir, scale=1, channels_sensors=[1, 3], type_disruptions=[None, None],
        ...
        self.count_dist = [0] * len(self.sensor_ids_list)
        self.count_no_dist = [0] * len(self.sensor_ids_list)
        self.disr_pattern = []
        for p in self.prop_disruptions:
            self.disr_pattern += [[True] * int(p * 100) + [False] * int((1-p) * 100)]
        np.random.shuffle(self.disr_pattern)
    ...
    def __getitem__(self, idx):
        ...
        for i, type_dist in enumerate(self.type_disruptions):
            if self.disr_pattern[i][(self.count_dist[i] + self.count_no_dist[i]) % 100]:
                self.count_dist[i] += 1
                if type(type_dist) == type([]):
                    imgs[i] = self.disturb_img(imgs[i], level=np.random.randint(6), type_dist=type_dist[np.random.randint(len(self.type_disruptions[0]))])
                else:
                    imgs[i] = self.disturb_img(imgs[i], level=self.level_disruptions[i], type_dist=type_dist)
            else:
                self.count_no_dist[i] += 1
                
        print(self.count_dist,self.count_no_dist, self.count_dist[0] / (self.count_dist[0] + self.count_no_dist[0]))
