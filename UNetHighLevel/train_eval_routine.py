...
from evaluate import evaluate, evaluate_high
...
def eval_network(net, dir_img: str, dir_mask: str,
    ...
    val_score = evaluate_high(net, eval_loader, device, channels_sensors)
...
if __name__ == '__main__':
    ...
    try:
        ...
        for rep in args.repeats:
            ...
            nets = []
            for i, n_channels_net in enumerate(args.channels_sensors):
                ...
                net = train_net(net=net,
                                ...
                                amp=args.amp,
                                channels_sensors=[args.channels_sensors[i]],
                                prop_disruptions=[args.disturb_prop_training[i]],
                                type_disruptions=[args.disturb_type_training[i]],
                                level_disruptions=[args.disturb_level_training[i]],
                                dir_img=dir_img_train,
                                dir_mask=dir_mask_train,
                                sensor_ids_list=[args.sensor_ids_list[i]])
                nets += [net]
            ...
            for type_sens in ["none", "blur", "snp_delta", "delete_pixel"]:
                ...
                for level_sens in range(6):
                    ...
                    eval_score[0,level_sens] = eval_network(nets, dir_img_test, dir_mask_test, type_disruptions=[type_sens, type_sens],
                                            prop_disruptions=[1, 1], level_disruptions=[level_sens, level_sens], 
                                            sensor_ids_list=args.sensor_ids_list, channels_sensors = args.channels_sensors)
            ...
            for type_sens1 in ["blur", "snp_delta", "delete_pixel"]:
                ...
                for level_sens in range(6):
                    print("Training:",args.saved_fn)
                    print("Eval: ",type_sens1, "none", level_sens, "Rep", rep)
                    eval_score[0,level_sens] = eval_network(nets, dir_img_test, dir_mask_test, type_disruptions=[type_sens1, "none"],
                                            prop_disruptions=[1, 1], level_disruptions=[level_sens, level_sens], 
                                            sensor_ids_list=args.sensor_ids_list)
                filename = f"{args.saved_fn}/eval_disturbances_sens1_{type_sens1}_sens2_none.npy"
                if os.path.exists(filename):
                    loaded_array = np.load(filename)
                    appended_array = np.concatenate((loaded_array,eval_score))
                    np.save(filename, appended_array)
            ...
            for type_sens2 in ["blur", "snp_delta", "delete_pixel"]:
                ...
                for level_sens in range(6):
                    print("Training:",args.saved_fn)
                    print("Eval: ","none", type_sens2, level_sens, "Rep", rep)
                    eval_score[0,level_sens] = eval_network(nets, dir_img_test, dir_mask_test, type_disruptions=["none",type_sens2],
                                            prop_disruptions=[1, 1], level_disruptions=[level_sens, level_sens], 
                                            sensor_ids_list=args.sensor_ids_list)
                filename = f"{args.saved_fn}/eval_disturbances_sens1_none_sens2_{type_sens2}.npy"
                if os.path.exists(filename):
                    loaded_array = np.load(filename)
                    appended_array = np.concatenate((loaded_array,eval_score))
                    np.save(filename, appended_array)
