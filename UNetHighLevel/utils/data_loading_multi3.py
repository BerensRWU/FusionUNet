...
class SyntheticDataset3(BasicDataset):
    ...
    def __getitem__(self, idx):
        ...
        if type(self.type_disruptions[0]) == type([]):
            
            if np.random.uniform(low=0.0, high=1.0) < self.prop_disruptions[0]:
                type_dist = ["delete_pixel", "blur", "snp_delta"][np.random.randint(3)]
                level = np.random.randint(1,6)
                random_number = np.random.uniform(low=0.0, high=1.0)

                if random_number < 0.333:
                    imgs[0] = self.disturb_img(imgs[0], level=level, type_dist=type_dist)
                elif random_number < 0.66666:
                    imgs[1] = self.disturb_img(imgs[1], level=level, type_dist=type_dist)
                else:
                    imgs[0] = self.disturb_img(imgs[0], level=level, type_dist=type_dist)
                    imgs[1] = self.disturb_img(imgs[1], level=level, type_dist=type_dist)
            ...
            for i, type_dist in enumerate(self.type_disruptions):
                if np.random.uniform(low=0.0, high=1.0) < self.prop_disruptions[i]:
